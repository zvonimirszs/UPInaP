...
public class RequestRecordsController : ControllerBase
{
    ...
    public ActionResult<RequestReadDto> CreateRequestForType(int requestTypeId, RequestCreateDto requestDto)
    {
        ...
        var articleNo = _repository.GetLegislationArticleNo();
        foreach (var item in articleNo)
        {
            legalRequest.ArticleNo.Add(item);
        }

        var legislationResponse = _repository.GetLegalLegislations(legalRequest);        

        var requestItem = _repository.GetRequestById(null, requestReadDto.Id);      
        if(requestItem == null)
        {
            //throw new ServiceException($"--> NIJE moguće upisati ZAHTJEV");
            Console.WriteLine($"--> NIJE moguće upisati UPIT");
        }
     
        if(legislationResponse == null)
        {
            return Ok(new { zahtjev = _mapper.Map<RequestReadDto>(requestItem), odgovor =  "Sadržaj zakonodastva nije moguće isporučiti"});  
        }
        return Ok(new { zahtjev = _mapper.Map<RequestReadDto>(requestItem), odgovor =  legislationResponse});
